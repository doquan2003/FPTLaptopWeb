...
public class AddCategoryServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (c != null) {
                request.setAttribute("error", "CategoryId " + categoryId + " already exist");
                request.getRequestDispatcher("AddCategory.jsp").forward(request, response);
                return;
            } else {
                Category cnew = new Category(categoryId, name, categoryStatus);
                cdao.insertCategory(cnew);
                System.out.println("Name " + cnew.getCategoryName());
                request.getRequestDispatcher("list-cate").forward(request, response);
            }
        } catch (Exception e) {
             request.setAttribute("error", "Id must interger number");
                request.getRequestDispatcher("AddCategory.jsp").forward(request, response);
