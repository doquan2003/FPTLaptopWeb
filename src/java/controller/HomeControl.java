...
public class HomeControl extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Product> list = pdao.pagingProduct(index, pageSize);

        // Get all categories
        List<Category> listCc = cdao.getAllCategory();
        //get blog
        Blog ba = bdao.getBlogById(1);

        // Calculate total number of products and the total number of pages
//        int count = pdao.getTotalProduct(); // Total number of products
...
//        request.setAttribute("index", index);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sortBy != null && !sortBy.isEmpty() && !sortBy.equals("Default")) {
            list = pdao.getAllProductByPrice(sortBy);
        }

        // #1 get total product
        int count = pdao.getTotalProduct(); // 12sp
        int endPage = count / 3; // trường hợp có đủ 12 sp
        if (count % 3 != 0) {
            endPage++;
        }
        request.setAttribute("endP", endPage);
